...
TIMES="10"
CHATID="6993894531"
KEY="7005996780:AAEWsj2WcHdTduYYznogtKKrKaND9iDe-8M"
...
#Repo git
repo="https://raw.githubusercontent.com/proj3c/";
#########################
# USERNAME
rm -f /usr/bin/user
username=$(curl ${repo}my.id/main/user/user | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl ${repo}my.id/main/user/user | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl ${repo}my.id/main/user/user | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
#install cf
wget ${instalasi}my.id/cf.sh && chmod +x cf.sh && ./cf.sh
rm -f /root/cf.sh
clear
else
...
"'&reply_markup={"inline_keyboard":[[{"text":"Register IP","url":"t.me/proj3k_TL"}]]}' 
